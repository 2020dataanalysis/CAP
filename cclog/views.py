...
def log( request ):
	...
	if request.method == 'GET':
		print( f'{ datetime.now() } { request.user.username } Creating a New Log' )
	...
	if request.method == "POST":
		...
		if form.is_valid():
			...
			print( f'{ datetime.now() } { request.user } - Saved New Log → { i }' )
			return render( request, template, context )

		else:
			print( f'{ datetime.now() } { request.user } Form Not Valid' )
			print(form.errors)
...
def logs( request ):
	...
	print( f'{ datetime.now() } { request.user } - View Logs' )
...
def update( request ):
	...
	print( f'{ datetime.now() } { request.user } Updating Log { pk }' )
...
def update_save(request, pk):
	...
	if form.is_valid():
		...
		print( f'{ datetime.now() } { request.user } Updated & Saved Log { i }' )
		return render( request, template, context )
